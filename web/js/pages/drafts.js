  ...
  async render() {
    return `
      <div class="section-title">
        ...
        <div>
          <button class="btn btn-primary btn-sm" onclick="DraftsPage.showGenerateModal()">+ Generate</button>
        </div>
      </div>
      <div class="filters">
      ...
      <div id="generate-modal-overlay" class="modal-overlay" style="display:none" onclick="DraftsPage.closeGenerateModal(event)">
        <div class="modal" onclick="event.stopPropagation()">
          ...
          <input type="text" id="gen-entry-id" placeholder="Entry ID" class="filter-input" style="margin-bottom:8px">
      ...
      el.innerHTML = data.tweets.map(t => `
        <div class="tweet-card" onclick="App.navigate('edit', '${t.id}')">
          <div class="tweet-card-header">
            <span>${t.status} (v${t.version})</span>
            <span>${fmtDate(t.created_at)}</span>
          </div>
          <div class="tweet-card-content">${escHtml(t.content)}</div>
          <div class="tweet-card-footer">
            <span>${t.content.length}/280</span>
            <span class="${t.content.length > 280 ? 'char-count over' : 'char-count'}"></span>
            <span>${t.provider_name || 'N/A'}</span>
            <span>$${t.cost_usd ? t.cost_usd.toFixed(6) : '0'}</span>
